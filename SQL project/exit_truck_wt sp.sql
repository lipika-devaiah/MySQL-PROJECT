-- exit_truck_weight store procedure
...
BEGIN
    ...
    -- Fetch entry
    SELECT entry_truck_wt, TankID, CustID, trx_date
    INTO v_entrywt, v_tankid, v_custid, v_trxdate
    FROM ci_truckload
    WHERE TruckNo = v_truckno AND exit_truck_wt IS NULL
    LIMIT 1;

    IF v_entrywt IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No pending truck entry';
    END IF;
    ...
    SET v_net = v_exitwt - v_entrywt;

    -- Check tank balance
    SELECT BalQty INTO v_bal FROM ci_TANK WHERE TankID = v_tankid;
    ...
    -- Update tank balance
    UPDATE ci_TANK SET BalQty = BalQty - v_net WHERE TankID = v_tankid;

    -- Log transaction
    INSERT INTO ci_tranlog (Opcode, CustID, TRX_DT, trx_qty)
    VALUES ('LOADTRUCK', v_custid, v_trxdate, v_net);
END;
//
DELIMITER ;
